...
type NicheFooterProps = {
  config: Pick<NicheConfig, "footerLinks" | "socialLinks" | "phoneNumber" | "phoneDisplay">
  dictionary: NicheDictionary["footer"]
}

export function NicheFooter({ config, dictionary }: NicheFooterProps) {
  const year = new Date().getFullYear()
  const legal = dictionary.legal.replace("{year}", String(year))

  const resolveLabel = (key: string) =>
    dictionary.links[key as keyof typeof dictionary.links] ?? key
      ...
      <div className="container space-y-8 overflow-hidden py-12">
        <nav className="-mx-5 -my-2 flex flex-wrap justify-center" aria-label="Footer">
          {config.footerLinks.map((link) => (
            <Button variant="ghost" key={`footer-${link.labelKey}`} asChild>
              <Link href={link.href} className="text-base leading-6 text-foreground">
        ...
        <p className="text-center text-sm text-muted-foreground">
          ...
          <a href={`tel:${config.phoneNumber}`} className="font-medium text-brand hover:underline">
            {config.phoneDisplay}
          </a>
        </p>
        ...
        <p className="text-center text-base leading-6 text-muted-foreground">{legal}</p>
      </div>
    </footer>
  )
}
